...
### 2.2 测试数据
- 默认管理员账号（执行 `backend/database/schema.sql` 后）:
...
```sql
-- 插入测试活动
INSERT INTO events (title, cover, description, start_time, end_time, place, limit, status, allowed_colleges, allowed_grades, creator_id) VALUES
...
('测试活动2：AI讲座', 'https://picsum.photos/seed/test2/640/360', '测试活动描述2：人工智能技术前沿讲座，由知名教授主讲。', '2024-12-26 14:00:00', '2024-12-26 16:00:00', '报告厅A', 150, 1, '计算机学院', '2024', 1),
('测试活动3：编程比赛', 'https://picsum.photos/seed/test3/640/360', '测试活动描述3：ACM程序设计竞赛，考验同学们的编程能力。', '2024-12-27 09:00:00', '2024-12-27 17:00:00', '实验室1', 50, 1, '信息学院', '', 1),
...
| **测试名称** | 管理员编辑活动功能 |
| **前置条件** | 管理员登录，已有活动 |
| **测试步骤** | 1. 在活动卡片点击"编辑活动"<br>2. 修改标题、人数上限或限制学院/年级<br>3. 点击保存 |
| **预期结果** | 1. 活动信息更新成功<br>2. 列表展示更新后的信息<br>3. 数据库字段同步更新 |
...
|-------------|--------|
| **测试名称** | 学院限制报名验证 |
| **前置条件** | 活动设置了允许学院为“计算机学院” |
| **测试步骤** | 信息学院学生尝试报名该活动 |
| **预期结果** | 返回403并提示不符合学院限制 |

#### 3.3.4 报名资格限制测试（年级）
| 测试用例编号 | TC-008-2 |
|-------------|--------|
| **测试名称** | 年级限制报名验证 |
| **前置条件** | 活动设置了允许年级为“2024” |
| **测试步骤** | 学号非 2024 开头的学生尝试报名 |
| **预期结果** | 返回403并提示不符合年级限制 |
...
#### 5.1.2 测试文件说明
测试文件已存在：`backend/test/api.test.js`，采用 ESM 模块写法：

```javascript
import request from 'supertest';
import app from '../app.js';

describe('校园活动报名系统 API 测试', () => {
  let adminToken;
  let studentToken;
  let testEventId;
